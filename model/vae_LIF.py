...
from base.spiking_LIF import Spiking_LIF, last_Spiking_LIF
from base.spiking_IF import Alpha 
...
class LIF_VAE(VanillaVAE): # 최종 본으로 시영 되고 있음 
    ...
    4. decoder_input은 Spiking_LIF로 변환
    ...
    def __init__(self, in_channels, latent_dim, T: int = 3) -> None:
        ...
        params = Params(self.fc_mu).get_params()
        print(params)
        self.fc_mu = last_Spiking_LIF(nn.Sequential(
            QuantizedFC(**params['linear']),Alpha()), T)
        params = Params(self.fc_var).get_params()
        self.fc_var = last_Spiking_LIF(nn.Sequential(
            QuantizedFC(**params['linear']),Alpha()), T)
        ...
        self.final_layer = last_Spiking_LIF(nn.Sequential(
            last_trans2d(**params['trans2d_2']), # 마지막은 8bit로
            QuantTanh()), T)
