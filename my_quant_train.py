...
def train(network, trainloader, opti, epoch, monitor):
    ...
    for batch_idx, (real_img, label) in enumerate(trainloader):         
        ...
        if batch_idx == len(trainloader)-1:
            ...
            torchvision.utils.save_image((real_img+1)/2, f'{monitor.checkpoint_dir}/imgs/train/epoch{epoch}_input.png')
            torchvision.utils.save_image((recons+1)/2, f'{monitor.checkpoint_dir}/imgs/train/epoch{epoch}_recons.png')
            monitor.writer.add_images('Train/input_img', (real_img+1)/2, epoch)
            monitor.writer.add_images('Train/recons_img', (recons+1)/2, epoch)
    ...
    with torch.no_grad():
        for batch_idx, (real_img, label) in enumerate(testloader):         
            ...
            if batch_idx == len(testloader)-1:
                ...
                torchvision.utils.save_image((real_img+1)/2, f'{monitor.checkpoint_dir}/imgs/test/epoch{epoch}_input.png')
                torchvision.utils.save_image((recons+1)/2, f'{monitor.checkpoint_dir}/imgs/test/epoch{epoch}_recons.png')
                monitor.writer.add_images('Test/input_img', (real_img+1)/2, epoch)
                monitor.writer.add_images('Test/recons_img', (recons+1)/2, epoch)
